...
async function getAll(request, reply) {
    try {
        const workouts = await workout_model_1.default.find().sort({ createdAt: -1 }); // newest first
        reply.send({ workouts });
    }
    catch (err) {
...
async function create(request, reply) {
    try {
        ...
        const existing = await workout_model_1.default.findOne({ name });
        if (existing) {
            return reply
                .status(409)
                .send({ error: "workout with this name already exists" });
        }
        const user = await workout_model_1.default.create({
            name,
            description,
            video_iframe
        });
